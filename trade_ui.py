...
Drop these into app.py to add a third tab or extend the Backtest tab
with the R-based trade log.
"""
...
def render_trade_dashboard(trade_log, de40_df, config) -> None:
    """
    ...
    ----------
    trade_log : TradeLog
        Output of `run_trade_engine`.
    de40_df : pd.DataFrame
        The DE40 OHLCV dataframe (for overlaying trades on price).
    config : Config
        The active engine config (for context display).
    """
    ...
    bh = (1 + de40_df["Close"].pct_change().fillna(0)).cumprod() * eq.iloc[0]
    ...
    tbl = trade_log.trades[available].iloc[::-1]  # newest first
