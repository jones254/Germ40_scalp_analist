...
with tab_live:
    ...
    # Pick the horizon matching your intended hold time, then read the
    # signal / EMA pair / holding window from config.forecasts[horizon].
    try:
    ...
    if _atr is not None and "de40" in data and len(data["de40"]) > 30:
        ...
        # ---- Horizon selector ---------------------------------------------
        # Default to "medium" — matches a 1-4h / multi-day swing.
        # For scalping use "short", for multi-day swing use "long".
        _horizon_labels = {
        ...
        # Forecast classifies Bullish / Bearish / Neutral.  We trade both
        # directions when the forecast is non-neutral.
        ...
        if side == 0 or atr_now <= 0:
            ...
            # The forecast is built from EMA fast/slow.  We mirror the trade-
            # engine math but scale the target to the horizon's slow-EMA
            # distance — a longer horizon deserves a wider, more realistic
            # target than a fixed 3× ATR.
            #
            # Sizing rule of thumb:
            #   short  → 1.5× ATR stop,  2.5× ATR target   (R:R ~1.7)
            #   medium → 1.5× ATR stop,  3.0× ATR target   (R:R 2.0)
            #   long   → 2.0× ATR stop,  5.0× ATR target   (R:R 2.5)
            _horizon_params = {
            ...
            # EMA-based bias: if EMA fast > EMA slow on a "Bullish" forecast,
            # we want the pullback to be toward (but not past) the slow EMA.
            # This makes the entry structural instead of arbitrary.
            e_fast = float(_ema(de40_now["Close"], f_ef).iloc[-1])
            e_slow = float(_ema(de40_now["Close"], f_es).iloc[-1])
            structural_pullback = e_slow if side == 1 else e_slow  # both pull to slow EMA
            entry_limit = price_now - side * pb_frac * atr_now
            # Don't let the limit sit on the wrong side of the slow EMA
            ...
            # Position size (1% risk on $10k — change live below if you want)
            equity       = 10_000.0
            risk_dollars = equity * 0.01
            size         = risk_dollars / stop_dist

            # Time-to-target in wall-clock terms (uses _humanize from above)
            bars_to_target = int(target_dist / max(atr_now, 1e-9) * f_es)
            # Rough: assume target is hit after ~EMA_slow bars at typical drift
            t_target = _humanize(int(f_es))
            t_stop   = _humanize(int(f_sl_equiv := max(5, f_es // 4)))
            ...
            # Slow EMA as a contextual line
            ema_slow_full = _ema(de40_now["Close"], f_es).tail(lookback)
